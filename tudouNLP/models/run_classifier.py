...
## Required parameters
flags.DEFINE_string('label_dict',None,'id2label.pkl name')
...
class classify_text(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        ...
        return self._create_examples(
            self._read_classify_data(os.path.join(data_dir, "train.txt")), "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_classify_data(os.path.join(data_dir, "dev.txt")), "dev")

    def get_test_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_classify_data(os.path.join(data_dir, "test.txt")), "test")

    def get_labels(self):
        """See base class."""
        return FLAGS.label_list
...
class tagging_processor(DataProcessor):
    ...
    def get_test_examples(self, data_dir):
        return self._create_example(
            self._read_data_tagging(os.path.join(data_dir, "test.txt")), "test")
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
    ...
    if FLAGS.task_name.lower() == 'tag' or FLAGS.task_name == 'ner':
        for (i, label) in enumerate(label_list, 1):
            label_map[label] = i

        # 保存label->index 的map
        if not os.path.exists(os.path.join('./', FLAGS.label_dict)):
            with codecs.open(os.path.join('./', FLAGS.label_dict), 'wb') as w:
...
def file_based_convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if FLAGS.task_name.lower() == 'tag' or FLAGS.task_name.lower() == 'ner':
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            if FLAGS.task_name.lower() == 'tag' or FLAGS.task_name == 'ner':
                def metric_fn(per_example_loss, label_ids, logits):
                    ...
                    precision = tf_metrics.precision(label_ids, predictions, num_labels+1,list(range(1,num_labels-1)), average="macro")
                    recall = tf_metrics.recall(label_ids, predictions, num_labels+1, list(range(1,num_labels-1)), average="macro")
                    f = tf_metrics.f1(label_ids, predictions, num_labels+1, list(range(1,num_labels-1)), average="macro")
                    return {
...
def main(_):
    ...
    if FLAGS.do_predict:
        if FLAGS.task_name.lower() == 'tag' or FLAGS.task_name.lower() == 'ner':
            ...
            with open(os.path.join('./', FLAGS.lable_dict), 'rb') as rf:
